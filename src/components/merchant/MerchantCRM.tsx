    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 pt-1 scrollbar-none -mx-1 px-1">
        <div className="text-slate-400 text-xs flex items-center gap-1 mr-1 shrink-0">
          ...
          <span className="font-bold text-slate-500 text-xs">Filtros:</span>
        </div>
        {filterOptions.map((f) => (
          <button
            key={f.id}
            onClick={() => setActiveFilter(f.id)}
            className={`py-2 px-3.5 rounded-xl text-xs font-semibold whitespace-nowrap transition min-h-[40px] flex items-center justify-center ${
              activeFilter === f.id
      ...
      {/* Customers List: Mobile Card View (md:hidden) & Desktop Table View (hidden md:block) */}
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
        {filteredCustomers.length === 0 ? (
          ...
          <>
            {/* 1. MOBILE RESPONSIVE CARDS (No horizontal scroll, fully fitted) */}
              ...
              {filteredCustomers.map((cust) => {
                ...
                return (
                  <div
                    ...
                    className="p-4 hover:bg-slate-50/80 active:bg-slate-100 transition cursor-pointer space-y-3"
                  >
                    {/* Top Row: Name, Avatar & Status Badge */}
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div className="w-9 h-9 rounded-2xl bg-indigo-50 text-indigo-700 font-black flex items-center justify-center text-sm shrink-0 border border-indigo-100">
                      ...
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold border shrink-0 ${statusBadge}`}
                      >
                        {statusLabel}
                      </span>
                    </div>

                    {/* Middle Row: Rating & Date Info */}
                    <div className="flex items-center justify-between bg-slate-50 p-2.5 rounded-xl text-xs text-slate-600">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 pt-0.5">
                      <a
                        ...
                        className="w-full sm:flex-1 py-2.5 px-3 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 border border-emerald-200/60 transition min-h-[44px]"
                      >
                      ...
                      <button
                        ...
                        className="w-full sm:flex-1 py-2.5 px-3 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 border border-indigo-200/60 transition min-h-[44px]"
                      >
            ...
            {/* 2. DESKTOP TABLE VIEW (For larger screens) */}
        ...
        {selectedCustomer && (
          <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-xs">
            <motion.div
              ...
              className="w-full sm:max-w-lg bg-white h-full shadow-2xl flex flex-col justify-between overflow-y-auto"
            >
              {/* Drawer Header */}
              <div className="p-4 sm:p-6 border-b border-slate-100 flex items-start justify-between bg-slate-50/50 sticky top-0 z-10">
                ...
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-slate-700 rounded-xl hover:bg-slate-200/60 transition min-w-[44px] min-h-[44px] flex items-center justify-center -mr-1"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-4 sm:p-6 space-y-5 flex-1">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3.5 bg-indigo-50/80 rounded-2xl border border-indigo-100">
                  <a
                    href={getWhatsAppLink(selectedCustomer.phone)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full sm:w-auto flex items-center justify-center gap-2 py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-xs transition min-h-[44px]"
                  >
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2 text-xs">
                    <span className="text-slate-600 font-semibold">Estado:</span>
                    <select
                      value={selectedCustomer.status}
                      onChange={(e) => handleUpdateStatus(e.target.value as Customer['status'])}
                      className="bg-white border border-slate-200 text-slate-800 text-xs font-bold rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500 min-h-[40px]"
                    >
